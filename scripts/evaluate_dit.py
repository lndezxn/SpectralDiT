...
from src.data.imagenet100 import build_imagenet100_image_dataloader
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a trained DiT with paper metrics.")
    ...
    parser.add_argument("--num-samples", type=int, default=None, help="Number of generated samples.")
    parser.add_argument("--num-real", type=int, default=None, help="Number of real reference images.")
    parser.add_argument(
        "--real-split",
        type=str,
        choices=("train", "test", "validation", "all"),
        default=None,
        help="Real reference split.",
    )
...
def build_real_loader(
    ...
    data_type = str(data_config.get("type", "cifar10"))
    if data_type == "imagenet100_latents":
        return build_imagenet100_image_dataloader(
    ...
    if split not in {"train", "test"}:
        raise ValueError("CIFAR-10 real split must be 'train' or 'test'.")
    dataset = build_cifar10_dataset(root=data_config["root"], train=split == "train")
    if num_real is None:
...
def main() -> None:
    ...
    data_type = str(config["data"].get("type", "cifar10"))
    if data_type == "imagenet100_latents":
        num_samples = args.num_samples if args.num_samples is not None else 50000
        num_real = args.num_real
        real_split = args.real_split or "all"
    else:
        num_samples = args.num_samples if args.num_samples is not None else 10000
        num_real = args.num_real if args.num_real is not None else 10000
        real_split = args.real_split or "test"
    batch_size = args.batch_size or int(config["eval"]["batch_size"])
    model_image_size = int(config["model"]["image_size"])
    in_channels = int(config["model"]["in_channels"])
    num_classes = int(config["model"]["num_classes"])
    metric_image_size = int(config["eval"].get("image_size", 256 if data_type == "imagenet100_latents" else model_image_size))
    ...
    ema_model.to(device)
    ema_model.eval()
    vae = load_vae(config["data"]["vae"], device) if data_type == "imagenet100_latents" else None

    real_loader = build_real_loader(config, real_split, num_real, batch_size)
    ...
    real_state = initialize_state(metric_image_size, device)
    generated_state = initialize_state(metric_image_size, device)
    ...
    with progress:
        total_real = len(real_loader.dataset)
        real_task = progress.add_task("Processing real images", total=total_real)
        ...
        generated_task = progress.add_task("Generating images", total=num_samples)
        for offset in range(0, num_samples, batch_size):
            ...
            samples = sample_euler(
                ...
                clamp=data_type != "imagenet100_latents",
            )
            if data_type == "imagenet100_latents":
                if vae is None:
                ...
                samples = decode_latents(
                    vae=vae,
                    latents=samples,
                    scaling_factor=float(config["data"]["scaling_factor"]),
    ...
    if args.output is not None:
        ...
        save_metrics(metrics, radial_spectrum, args, config_path, num_samples, total_real, real_split, args.output)
